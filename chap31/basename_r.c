/* basename_r.c - thread-safe implementation of the basename(3) function.
 *
 * The basename(3) function returns the name of a file, i.e., the component
 * past the last slash '/' character. See the man page for further explanation
 * and edge cases.
 *
 * In Linux, the basename(3) function is already thread-safe. However, SUSv3
 ...
 * Usage
 *
 *    $ ./basename_r <main-thread-path> <thread1-path> [...]
 *
...
static char *basename_r(char *path);
...
main(int argc, char *argv[]) {
	...
	main_thread_basename = basename_r(argv[1]);
...
static void
basename_data_destructor(void *buffer) {
	free(buffer);
}

static void
create_basename_key(void) {
	int s = pthread_key_create(&basename_key, basename_data_destructor);
	if (s != 0)
		pthread_pexit(s, "pthread_key_create");
}

/* retrieves thread-specific data for the basename_r function for the running
 * thread */
static void *
get_basename_specific() {
	int s;
	s = pthread_once(&once_control, create_basename_key);
	if (s != 0)
		pthread_pexit(s, "pthread_once");

	void *buf = pthread_getspecific(basename_key);
	if (buf == NULL) {
		buf = malloc(PATH_MAX);
		if (!buf)
			pexit("malloc");
...
static char *
basename_r(char *path) {
	char *buf = get_basename_specific();
	int pathlen = strlen(path);

	/* if path is NULL or the empty string, basename should return "." */
